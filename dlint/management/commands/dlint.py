...
from dlint import resources


class Command(BaseCommand):

    option_list = BaseCommand.option_list + (
        make_option("-w", "--show-warnings", action="store_true", dest="show_warnings", default=False, help='Shows warnings.'),
    )

    def handle(self, *args, **options):

        sources = list(resources.templates.items())
                ...
                if token.token_type == TOKEN_VAR:
                    ...
                    for content in contents[1:]:
                        try:
                            ...
                            # a TemplateSyntaxError, that will be ignored.
                            pass
